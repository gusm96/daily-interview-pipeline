...
def test_is_bot_or_self_detects_bot(monkeypatch):
    ...
    assert slack_client.is_bot_or_self({"bot_id": "B1", "user": "U2"}) is True
    assert slack_client.is_bot_or_self({"user": "UBOT"}) is True
    assert slack_client.is_bot_or_self({"subtype": "message_changed", "user": "U2"}) is True
    assert slack_client.is_bot_or_self({"user": "UHUMAN", "text": "hi"}) is False


def test_extract_user_answer_returns_text(monkeypatch):
    monkeypatch.setenv("SLACK_BOT_USER_ID", "UBOT")
    event = {"user": "UHUMAN", "text": "내 답변", "thread_ts": "123.45", "ts": "123.46"}
    assert slack_client.extract_user_answer(event) == "내 답변"
...
def test_extract_user_answer_ignores_top_level(monkeypatch):
    ...
    assert slack_client.extract_user_answer(event) is None
...
def test_slack_post_message_calls_sdk(monkeypatch):
    ...
    with patch("slack_client.WebClient", return_value=fake_client):
        slack_client.slack_post_message("C1", "안녕", thread_ts="123.4")
...
def test_slack_get_thread_parent_returns_text(monkeypatch):
    ...
    with patch("slack_client.WebClient", return_value=fake_client):
        text = slack_client.slack_get_thread_parent("C1", "123.4")
...
def test_extract_user_answer_keeps_plain_thread_reply(monkeypatch):
    ...
    monkeypatch.setenv("SLACK_BOT_USER_ID", "UBOT")
    event = {"user": "UHUMAN", "text": "안녕하세요", "thread_ts": "1", "ts": "2"}
    assert slack_client.extract_user_answer(event) == "안녕하세요"
