...
(선택: `GEMINI_MODEL`, `SLACK_CHANNEL_ID`, `SLACK_ALLOWED_USER_IDS`, `LOG_LEVEL`)

- `LOG_LEVEL`: 루트 로거 레벨. 미설정 시 `INFO`. 로그는 stdout에 JSON 한 줄(`severity`/`message`/`logger`)로 나가 Cloud Logging이 구조화 로그로 파싱한다.
